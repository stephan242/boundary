...
func CreateNewAccountApi(t testing.TB, ctx context.Context, client *api.Client, loginName string) (string, string) {
	c, err := loadConfig()
	...
	t.Logf("Create Account: %s", newAccountId)
	t.Cleanup(func() {
		_, err := aClient.Delete(ctx, newAccountId)
		require.NoError(t, err)
	})
